...
\newcommand{\MySectionSpacing}{1}
%\newcommand{\MySectionSpacing}{10}
\newcommand{\MySectionFontSize}{35}
...
\title
{
\fontsize{120}{120}\selectfont%1st number is font, 2nd is spacing 
\begin{center}
...
My previous work in 2018~\cite{Ehrenborg_2018}
focused on characterizing 
integer solutions~to
$a^2 + b^2 + c^2 + d^2 = e^2$.
Current research explores the polynomial solutions
in $ \Zzz[x,y,z,w] $ to
\begin{equation}
\label{eq:e^n}
a^2 + b^2 + c^2 + d^2 = e^n,
...
I characterized integer
solutions geometrically and
by viewing~\eqref{eq:e^n} as a product of two quaternions. This gave
rise to polynomial solutions to
$a^2 + b^2 + c^2 + d^2 = e^2$
with $e = x^2 + y^2 + z^2 + w^2$.
The paper found that an upper bound for the number
of such solutions is $57$.
...
Recall the \emph{complex numbers} $\Ccc$ are a $2$-dimensional
field extension of $\Rrr$.
A complex number is of the form $x + iy$, where $i = \sqrt{-1}$
and $x,y \in \Rrr$.
The \emph{conjugate} of a complex number $ z = x + iy$ is $ \bar{z} = x - iy$, and  
the \emph{norm} is $ \N(z) = z \bar{z} = x^2 + y^2$.  

The \emph{Gaussian integers} $ \Zzz[i] = \{ x + iy: x,y \in \Zzz \} $
are a subring of $\Ccc$.


The \emph{quaternions} $\mathscr{Q}$,
discovered by Sir William Rowan Hamilton
in 1844~\cite{Hamilton},
are a
$4$-dimensional division ring extension of $\Rrr$.
A quaternion has the form $x + iy + jz + kw$, with
...
ki = -ik = j.
\end{align*}
Addition is component-wise, and multiplication is
...
Define an equivalence relation, denoted by $ \simeq $, on $p$-tuples
$ \tau $ 
...
\label{eq:Pythagorean}
    ...
    = (x^2 + y^2)^2.
\end{equation}
This follows by taking the norm of
...
\begin{remark}
\bf
Equation~\eqref{eq:Pythagorean} dates back to Euclid~\cite{Euclid}. 
\end{remark}
...
Take the norms of the products
$
    (x + iy)(x + iy)(x + iy) 
...
When $ p = m = 2$, Definition~\ref{def:general} has an alternate form.
...
By Lemma~\ref{lemma_equivalence},
...
The order of the stabilizer is $8$, as the two variables can be rearranged and their
...
This follows similarly. \qedhere
\vanish{
There are $ z + 1 $ equivalence classes. We choose $ z + 1 $
representatives of the form $ (x - iy)^j (x + iy)^k $, with
$ 0 \leq j \leq \myfrac{n}{2}$ and $ j + k = n $.
...
lack multiplicative commutativity, so the
proof technique in Theorem~\ref{thm:2D_equivalence}
cannot be used.
Thus experimental data becomes more important.
...
\begin{tabular}{ r || m{1cm} | m{1cm} | m{1cm} | m{1cm} | m{1cm} | m{1cm} | m{1cm} | m{1cm} | }
...
\begin{center}
{\fontsize{\MySectionFontSize}{\MySectionSpacing}\selectfont \textbf
 {Proof of Conjecture~\ref{conjecture_Ehrenborg-Leep} when $n=1$}
}
\end{center}

\noindent
The following conjecture was made in
collaboration with Professor Leep.
\begin{conjecture}
...
Assume the coordinate containing $ \pm x $ also contains
...
with respect to any variable of $2$, any monomial similar to $ A^2 $ must itself be
a square of a monomial similar to~$ A $. In this case, the coefficients are
positive and will not cancel.
Thus there can be no such monomial~$ A $, so the only monomials
...
\begin{center}
{\fontsize{\MySectionFontSize}{\MySectionSpacing}\selectfont \textbf
 {Enumerative Consequences of Conjecture~\ref{conjecture_Ehrenborg-Leep}} 
}
\end{center}
